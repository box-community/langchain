...
def test_folder_load(mocker: MockerFixture) -> None:
    file_list = []
    file_list.append(FileFull(type="file", id="id"))
    file_list.append(FileFull(type="folder", id="id"))

    mocker.patch(
        "langchain_box.utilities.BoxAPIWrapper.get_folder_items", return_value=file_list
    )
    mocker.patch(
